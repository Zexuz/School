...
namespace PointManager.Models
{
    public class Camera
    {
        ...
        public Point3D Look
        {
            ...
            {
                const int dist = 3;
                double X1 = Math.Sin(degH * halfPi) * dist, Z1 = Math.Cos(degH * halfPi) * dist;
                return new Point3D() { Y = (Math.Sin(degV * halfPi) * dist), Z = (Math.Cos(degV * halfPi) * Z1), X = (Math.Cos(degV * halfPi) * X1) };
            }
        }

        public void Move(double Distance) { _Position.X += Math.Sin(degH * halfPi) * Distance; _Position.Z += Math.Cos(degH * halfPi) * Distance; }
